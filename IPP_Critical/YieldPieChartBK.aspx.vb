...
Partial Class YieldPieChart
    ...
    Private Sub pageInit(ByVal part_id As String, ByVal item As String, ByVal week As String, ByVal weekIn As String, ByVal product As String, ByVal plant As String)
        ...
        Try
            ...
            sqlStr += plantStr
            sqlStr += "and a.Part_Id=b.Part_Id "
            sqlStr += "and b.Production_Type='{1}' "
            sqlStr += "and WW={2} "
            ...
            Bump_IPQC_Detail(conn, part_id, item, week, weekIn)
    ...
    Private Sub Bump_IPQC_Detail(ByRef conn As SqlConnection, ByVal part_id As String, ByVal item As String, ByVal week As String, ByVal weekIn As String)
        ...
        If (item.ToUpper).IndexOf("BUMP") >= 0 Or (item.ToUpper).IndexOf("IPQC") >= 0 Then
            ...
            If (item.ToUpper).IndexOf("IPQC") >= 0 Then
                failType = "IPQC"
            End If

            Try
                ' 取得最新一週的 Yield 順序的 Items
                sqlStr = "select Fail_Mode, ROUND(convert(float,SUM(Fail_Count))/SUM(Original_Input_QTY), 4) as YIELD_VALUE " +
                ...
                sqlStr = "Select WW, Fail_Mode, ROUND(convert(float,SUM(Fail_Count))/SUM(Original_Input_QTY), 4) as YIELD_VALUE " +
                         "From dbo.BinCode_Detail_Daily_Lot " +
                         "Where 1=1 " +
                         "And category='" + failType + "' " +
                         "And production_type='" + part_id + "' " +
                ...
                ' 取得 Lot 的 RowData 
                'LotDT = New DataTable()
                'sqlStr = "SELECT WW, Fail_Mode, ROUND(convert(float,SUM(Fail_Count))/SUM(Original_Input_QTY), 5) AS VALUE " +
                '         "From dbo.BinCode_Detail_Daily_Lot " +
                '         "Where 1=1 " +
                '         "And category='" + failType + "' " +
                '         "And production_type='" + part_id + "' " +
                ...
                'sqlStr = "select WW, Lot_Id, DataTime, fail_mode, Fail_Count, Original_Input_QTY as InCount, ROUND(Fail_ratio, 5) as fail_value " +
                '         "from dbo.BinCode_Detail_Daily_Lot " +
                '         "where 1=1 " +
                '         "and category = '" + failType + "' " +
                '         "And production_type='" + part_id + "' " +
                ...
                sqlStr += "FROM "
                sqlStr += "( "
                sqlStr += "SELECT WW, Fail_Mode, ROUND(convert(float,SUM(Fail_Count))/SUM(Original_Input_QTY), 4) AS VALUE "
                ...
                If yieldDT.Rows.Count > 0 And ItemDT.Rows.Count > 0 Then
                    Bump_IPQC_Chart(yieldDT, ItemDT)
    ...
    Private Sub Bump_IPQC_Chart(ByRef DtSet As DataTable, ByRef setupDT As DataTable)
